          ...
          <q-card class="my-card col-lg-3 col-md-4 col-sm-7" flat bordered>
            <q-card-section horizontal>
              <q-card-section class="q-pt-xs">
                <div class="text-h6 q-mt-sm" style="font-size: 14px">Data Pengeluaran belum dibayar</div>
              ...
              <q-card-section horizontal>
                <q-card-section class="col-12 q-pa-sm">
                  ...
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td key="idPengeluaran" :props="props">{{props.row.idPengeluaran}}</q-td>
                      ...
                      <q-td key="barang" :props="props">
                        <q-btn @click="showDetail(props.row._id, props.row.grandTotal)" outline color="primary" label="detail" size="sm" class="q-ml-sm" />
                      </q-td>
        ...
        <q-dialog v-model="detail.visible">
          <q-card>
            <q-card-section>
              ...
              <p class="text-caption q-ml-md">supplier: <b>{{detail.namaBarang}}</b></p>
            </q-card-section>
...
export default {
  ...
  data () {
    return {
      ...
      totalLunas: 0,
      columns: [
        { name: 'idPengeluaran', required: true, label: 'ID Pengeluaran', align: 'left', field: 'idPengeluaran', sortable: true },
        { name: 'barang', required: true, label: 'Barang', align: 'left', field: 'barang', sortable: true },
      ...
      detail: {
        ...
        columns: [
          { name: 'idPengeluaran', required: true, label: 'ID Pengeluaran', align: 'left', field: 'idPengeluaran', sortable: true },
          { name: 'namaBarang', required: true, label: 'Barang', align: 'left', field: 'namaBarang', sortable: true },
          { name: 'hargaBarang', required: true, label: 'Harga Barang', align: 'left', field: 'hargaBarang', sortable: true },
          { name: 'jumlahPengeluaran', required: true, label: 'Jumlah Pengeluaran', align: 'left', field: 'jumlahPengeluaran', sortable: true },
          { name: 'total', required: true, label: 'Total', align: 'left', field: 'total', sortable: true },
          { name: 'grandTotal', required: true, label: 'Grand Total', align: 'left', field: 'grandTotal', sortable: true }
  ...
  methods: {
    ...
    showDetail (id, grandTotal) {
      try {
        this.$api.get('pengeluaran/detail/' + id)
          .then(res => {
            if (res.data.status !== true) {
              this.$showNotif(res.data.message, 'negative')
            } else {
